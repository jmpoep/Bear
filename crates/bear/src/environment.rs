...
use intercept::environment::{KEY_INTERCEPT_STATE, KEY_OS__PATH, PreloadState, insert_to_path};
#[cfg(target_os = "macos")]
use intercept::environment::{KEY_OS__MACOS_FLAT_NAMESPACE, KEY_OS__MACOS_PRELOAD_PATH};
use intercept::installation::InstallationLayout;
use intercept::supervise;
use intercept::wrapper::{WrapperDirectory, WrapperDirectoryBuilder, WrapperDirectoryError};
...
pub enum ConfigurationError {
    ...
    InstallationLayout(#[from] intercept::installation::LayoutError),
}
...
/// database generation.
///
/// This lives in `bear` (not `intercept`) because it depends on
/// [`args::BuildCommand`], which carries `clap` into the build graph; keeping it
/// here lets `intercept::Execution` stay `clap`-free.
